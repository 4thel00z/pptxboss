...
Notes live in a Notes Slide part related from the slide. The reader takes
the `body` placeholder of that part; when there is none, every text shape
other than the slide image and furniture placeholders.
...
into groups. `Shape::placeholder` carries the placeholder kind and index;
...
Runs carry the relationship id of a click hyperlink in `RunProps::hyperlink`;
